...
const sessionCsrf = () => new Response(JSON.stringify({ csrf_token: "test-csrf" }), { status: 200 });
...
test("로그인한 직원은 자신의 정보를 확인한다", async () => {
  vi.spyOn(globalThis, "fetch")
    ...
    .mockResolvedValueOnce(sessionCsrf());
  render(<App />);
...
test("로그인한 직원은 로그아웃할 수 있다", async () => {
  ...
  expect(await screen.findByRole("heading", { name: "로그인" })).toBeInTheDocument();
  expect(fetchMock.mock.calls[2][0]).toBe(`${API_BASE_URL}/auth/logout`);
...
test("전화번호와 주소를 수정한다", async () => {
  const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(new Response(JSON.stringify(employee), { status: 200 })).mockResolvedValueOnce(sessionCsrf()).mockResolvedValueOnce(new Response(JSON.stringify({ ...employee, phone: "010-9999-9999" }), { status: 200 }));
  render(<App />);
  fireEvent.change(await screen.findByLabelText("전화번호"), { target: { value: "010-9999-9999" } });
  fireEvent.click(screen.getByRole("button", { name: "저장" }));
  await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(3));
  expect(fetchMock.mock.calls[2][1]).toEqual(expect.objectContaining({ method: "PATCH", headers: expect.objectContaining({ "X-CSRF-Token": "test-csrf" }) }));
  expect(await screen.findByText("정보가 저장되었습니다.")).toBeInTheDocument();
});

test("저장 시 세션이 만료되면 로그인 화면으로 이동한다", async () => {
  vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(new Response(JSON.stringify(employee), { status: 200 })).mockResolvedValueOnce(sessionCsrf()).mockResolvedValueOnce(new Response(null, { status: 401 }));
  render(<App />);
...
test("관리자는 확인 후 직원을 퇴사 처리하고 상태를 확인한다", async () => {
  ...
  expect(fetchMock.mock.calls[5][0]).toBe(`${API_BASE_URL}/admin/employees/id/terminate`);
  expect(fetchMock.mock.calls[5][1]).toEqual(expect.objectContaining({ method: "POST", headers: { "X-CSRF-Token": "test-csrf" } }));
});
...
test("관리자는 이름 제안을 수정해 Background Check를 요청한다", async () => {
  ...
  const request = fetchMock.mock.calls[6];
